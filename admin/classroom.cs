    ...
    public partial class classroom : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < classroomlist.Rows.Count; i++)
            {
                try
                {
                    Method.updateclassroom_admin(classroomlist.Rows[i].Cells["crid"].Value.ToString(), classroomlist.Rows[i].Cells["roomnumber"].Value.ToString(), classroomlist.Rows[i].Cells["state"].Value.ToString());
